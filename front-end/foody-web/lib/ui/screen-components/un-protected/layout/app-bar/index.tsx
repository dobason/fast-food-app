...
// Icons
import { CartSvg, CircleCrossSvg, ClockSvg, LocationSvg, SearchSvg } from '@/lib/utils/assets/svg';
...
import { deleteSearchedKeywords, getSearchedKeywords } from '@/lib/utils/methods';

// Constnats
import { languageTypes, USER_CURRENT_LOCATION_LS_KEY } from '@/lib/utils/constants';
...
const AppTopbar = ({ handleModalToggle }: IAppBarProps) => {
  ...
  const formattedSubtotal =
    cartCount > 0 ? `${CURRENCY_SYMBOL}${calculateSubtotal()}` : `${CURRENCY_SYMBOL}0`;

  // Handlers
  const onInit = () => {
    const current_location_ls = onUseLocalStorage('get', USER_CURRENT_LOCATION_LS_KEY);
    const user_current_location = current_location_ls ? JSON.parse(current_location_ls) : null;
    ...
    const selectedAddress = profile?.addresses.find((address) => address.selected);
            ...
            <div className="flex items-center justify-between w-full h-16 gap-2 flex-wrap md:flex-nowrap">
              ...
              <div className="flex items-center gap-2 flex-shrink-0 cursor-pointer">
                {!isSearchFocused && (
                  ...
                  >
                    <Logo className="w-32 h-auto" fillColor="#000000" darkmode="#FFFFFFFF" />
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-end gap-2 flex-shrink-0">
                ...
                {!authToken && !isSearchFocused ? (
                  ...
                  >
                    ...
                    <FontAwesomeIcon icon={faChevronDown} width={12} hanging={12} color="#94e469" />
                    <Menu
                      className="
                     dark:bg-gray-800
                     dark:text-white
