...
// Methods
import { calculateAmount, calculateDistance, checkPaymentMethod } from '@/lib/utils/methods';
...
export default function OrderCheckoutScreen() {
  ...
  const [paymentMethod, setPaymentMethod] = useState(PAYMENT_METHOD_LIST[0].value);
  ...
  const { CURRENCY_SYMBOL, CURRENCY, DELIVERY_RATE, COST_TYPE, SERVER_URL } = useConfig();
  ...
  const { data: restaurantData } = useRestaurant(restaurantId || '') || {
    data: restaurantFromLocalStorage ? JSON.parse(restaurantFromLocalStorage) : null,
    loading: false,
  };
  ...
  const [placeOrder, { loading: loadingOrderMutation }] = useMutation(PLACE_ORDER, {
    onCompleted,
    onError,
    update,
  });
  const [verifyCoupon, { loading: couponLoading }] = useMutation(VERIFY_COUPON, {
  ...
  function validateOrder() {
    ...
    const delivery = isPickUp ? 0 : deliveryCharges;
    if (
      Number(calculatePrice(delivery, true)) < (finalRestaurantData.restaurant.minimumOrder || 0)
    ) {
      showToast({
    ...
    if (!profile) {
      showToast({
        ...
        message: "Your profile information couldn't be loaded. Please try again or login again.",
        type: 'error',
      });
  ...
  function onError(error: ApolloError) {
    showToast({
      title: 'Error',
      message:
        error.graphQLErrors[0]?.message || error?.networkError?.message || 'Something went wrong',
      type: 'error',
    });
  }
        ...
        <div className="max-w-6xl md:pt-10 p-4 md:p-0 lg:flex lg:space-x-4">
          <div className="lg:w-3/4 md:mr-40 md:rtl:ml-40">
            ...
            <div className="flex justify-between bg-gray-100 dark:bg-gray-800 rounded-full p-2 mb-6">
              <button
                className={`w-1/2 ${
                  deliveryType === 'Delivery' ? 'bg-[#5AC12F]' : 'bg-gray-100 dark:bg-gray-700'
              ...
              <button
                className={`w-1/2 ${
                  deliveryType === 'Pickup' ? 'bg-[#5AC12F]' : 'bg-gray-100 dark:bg-gray-700'
                ...
                <div className="flex items-center">
                  ...
                  <p className="text-gray-900 dark:text-gray-100 leading-4 sm:leading-5 tracking-normal font-inter text-xs sm:text-sm md:text-sm align-middle">
                    <span className="font-semibold">
                      {' '}
                      {deliveryType === 'Pickup' ? t('pickup_label') : t('delivery_label')}{' '}
                    </span>
                    <span className="font-normal">{t('in_10_20_min_label')} </span>
                    <span className="font-semibold">{userAddress?.deliveryAddress}</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 pt-4 px-3  pb-2 rounded-lg mb-4 w-full ">
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <div className="flex flex-col items-start">
                          {item?.optionTitles?.map((optionTitle, optionIndex) => {
                            return (
                              <p
              ...
              <button
                ...
                onClick={() => {
                  const currentShopType = onUseLocalStorage('get', 'currentShopType');
                  const restaurantId = onUseLocalStorage('get', 'restaurant');
                  const restaurantSlug = onUseLocalStorage('get', 'restaurant-slug');
                  router.replace(`/${currentShopType}/${restaurantSlug}/${restaurantId}`);
                }}
              >
              ...
              <div className="bg-white dark:bg-gray-900 mb-6 w-full">
                ...
                <div className="border border-gray-300 dark:border-gray-700 rounded-lg p-5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {tipData?.tips.tipVariations.map((tip: string, index: number) => (
                      <button
                        key={index}
                        className={`text-[12px] ${
                          selectedTip === tip
            ...
            >
              ...
              {isCouponApplied && (
                <div className="flex justify-between mb-1 text-xs lg:text-[14px]">
                  ...
                  <span className="font-inter text-gray-900 dark:text-white leading-5">
                    {`-${CURRENCY_SYMBOL} ${(
                      Number(calculatePrice(0, false)) - Number(calculatePrice(0, true))
                    ).toFixed(2)}`}
                  </span>
                </div>
              )}
