...
export default function RestaurantDetailsScreen() {
  ...
  // Fetch restaurant data (skip if using mock)
  const { data: realData, loading: realLoading } = useRestaurant(
    USE_MOCK_DATA ? '' : id,
    USE_MOCK_DATA ? '' : decodeURIComponent(slug)
  );
  ...
  const data = USE_MOCK_DATA ? mockRestaurantDetail : realData;
  const loading = USE_MOCK_DATA ? false : realLoading;
  ...
  const deals = useMemo(() => {
    const filteredDeals =
      (allDeals || [])
        .filter((c: ICategory) => {
          ...
          const categoryMatches = c.title.toLowerCase().includes(filter.toLowerCase());
    ...
    const popularFoods = allFoods.filter((food: IFood) => popularDealsIds?.includes(food._id));
    ...
    return popularDealsCategory ? [popularDealsCategory, ...filteredDeals] : filteredDeals;
      ...
      <div className="bg-gray-50 dark:bg-gray-800 shadow-[0px_1px_3px_rgba(0,0,0,0.1)] p-3 h-[80px] flex justify-between items-center">
        <PaddingContainer>
          <div className="flex flex-wrap items-center gap-4 sm:gap-6">
            ...
            <span className="flex items-center gap-2 text-gray-600 dark:text-gray-300  font-inter font-normal text-sm sm:text-base md:text-lg leading-5 sm:leading-6 md:leading-7 tracking-[0px] align-middle">
              <RatingSvg />
              {loading ? <Skeleton width="1rem" height="1.5rem" /> : headerData.averageReview}
            </span>
            ...
            >
              <InfoSvg />
              {loading ? <Skeleton width="10rem" height="1.5rem" /> : t('see_more_information')}
            </a>
            ...
            >
              <ChatSvg />
              {loading ? <Skeleton width="10rem" height="1.5rem" /> : t('see_reviews')}
            </a>
          </div>
        </PaddingContainer>
      </div>
