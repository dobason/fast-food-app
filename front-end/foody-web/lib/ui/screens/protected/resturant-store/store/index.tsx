...
export default function StoreDetailsScreen() {
  ...
  const { data: categoriesSubCategoriesList, loading: categoriesSubCategoriesLoading } = useQuery(
    GET_CATEGORIES_SUB_CATEGORIES_LIST,
    {
      variables: {
        storeId: id,
  ...
  const { data: popularSubCategoriesList } = useQuery(GET_POPULAR_SUB_CATEGORIES_LIST, {
    variables: {
      restaurantId: id,
    },
  });
  ...
  // Constants
  const allDeals = data?.restaurant?.categories?.filter((cat: ICategory) => cat.foods.length);
  ...
  const handleScroll = (id: string, isParent = true, offset: number = 120) => {
    ...
    if (isParent) {
      ...
      const sliderSubCategories =
        menuItems?.find((item: ICategoryDetailsResponse) => toSlug(item.label) === id)?.items || [];
  ...
  const handleOpenFoodModal = (food: IFood) => {
    ...
    if (!restaurantInfo?.isAvailable || !restaurantInfo?.isActive) {
            ...
            <div className="w-full md:w-[80%]">
              <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                ...
                <span className="flex items-center gap-1 text-gray-600 dark:text-gray-300 font-inter font-normal text-sm sm:text-base md:text-lg leading-5 sm:leading-6 md:leading-7 tracking-[0px] align-middle">
                  ...
                  {loading ? <Skeleton width="2rem" height="1.5rem" /> : headerData.deliveryTime}
                  <span>mins</span>
                </span>
                ...
                <span className="flex items-center gap-2 text-gray-600 dark:text-gray-300 font-inter font-normal text-sm sm:text-base md:text-lg leading-5 sm:leading-6 md:leading-7 tracking-[0px] align-middle">
                  ...
                  {loading ? <Skeleton width="2rem" height="1.5rem" /> : headerData.averageReview}
                </span>
          ...
          <div className="p-3  w-full flex flex-col md:hidden gap-2 items-center justify-between">
            ...
            >
              <ul className="flex space-x-4 items-center w-max flex-nowrap">
                {menuItems?.map((category: ICategoryDetailsResponse, index: number) => {
                  ...
                  return (
                    <li key={index} className="shrink-0">
                      <button
                        className={`bg-${
                          selectedCategory === _slug ? '[#F3FFEE]' : 'gray-100'
                        } text-${
                          selectedCategory === _slug ? '[#5AC12F]' : 'gray-600'
                    ...
                    (sub_category: ICategoryDetailsResponse, index: number) => {
                      ...
                      return (
                        <li key={index} className="shrink-0">
                          <button
                            className={`bg-${
                              selectedSubCategory === _slug ? '[#F3FFEE]' : 'gray-100'
                            } text-${
                              selectedSubCategory === _slug ? '[#5AC12F]' : 'gray-600'
                            } rounded-full px-3 py-2 text-[10px] sm:text-sm md:text-base font-medium whitespace-nowrap`}
          ...
          <div className="flex flex-col md:flex-row w-full">
            ...
            <div className="w-full md:w-4/5 p-3 h-full overflow-y-auto">
              {deals.map((category: ICategoryV2, catIndex: number) => (
                <div key={catIndex} className="mb-4" id={toSlug(category.title)}>
                  ...
                  {category.subCategories.map(
                    (subCategory: ISubCategoryV2, subCatIndex: number) => (
                      <div key={subCatIndex} className="mb-4" id={toSlug(subCategory.title)}>
                     ...
                     `}
                            ...
                            >
                              ...
                              <CustomDialog
                                ...
                                visible={
                                  isModalOpen.value && isModalOpen.id === meal?._id?.toString()
                                }
